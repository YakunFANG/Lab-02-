...
class Parser(val source: SourceFile):
  ...
  private[parsing] def binding(initializerIsExpected: Boolean = true): Binding =
     val let = expect(K.Let)
      val iden = expect(K.Identifier)
      peek match
        case Some(Token(K.Colon, _)) =>
          expect(K.Colon)
          val tpe = expect(K.Type)
          expect(K.Eq)
          val expre = expression()

          Binding(iden, Some(tpe)), Some(expre), let.site.extendedToCover(expre.site.end))
        case Some(Token(K.Eq, _)) =>
          expect(K.Eq)
          val expre = expression()

          Binding(iden, None, Some(expre), let.site.extendedToCover(expre.site.end))
        case _ =>
          recover(ExpectedTree("binding declaration", emptySiteAtLastBoundary), ErrorTree.apply)


  /** Parses and returns a function declaration. */
  private[parsing] def function(): Function =
      val fun = expect(K.Fun)                     /* [List] [ is inside the List or meaning of being optional paraList ?? */
      val functiden = functionIdentifier()        /* peek == expression(), taken or not taken?*/
      expect(K.LParen)
      peek match
      ...
      /* peek == xxx equal to peek matches xxx ?*/
      
      Function(functiden, None, None, None, expr, fun.site.extendedToCover(RBrace.site.end))
  ...
  private[parsing] def typeDeclaration(): TypeDeclaration =
    val ttype = expect(K.Type)
    val identi = expect(K.Identifier).toString()
    expect(K.Eq)
    val tpee = expect(K.Type)

    TypeDeclaration(identi, Nil, tpee, ttype.site.extendedToCover(tpee.site.end))
  ...
  private[parsing] def conditional(): Expression =
    ...
    val if_token = expect(K.If) /* The site part is just for returning where the error is, not that important */
    val if_expr = expression()
    expect(K.Then)
    val then_expr = expression()
    expect(K.Else)
    val else_expr = expression()
    Conditional(if_expr, then_expr, else_expr, if_token.site.extendedToCover(else_expr.site.end)) /* in this case, it will create a sourcespan from the start of the token stream to the end of whole token stream) */
